...
RSamplePlots <- function(plotdata, nsamples, 
  ...
  if (shape == "rectangle"){
    if (length(size) != 2 | size[1] > dimx[2] | size[2] > dimy[2]){
      stop("Retangular sample needs two dimensions smallers then plot size.") #compare with total area
    }
    halfs <- size/2
  }
  if (shape == "circle"){
  ...
  while (nsover > 0){
    ...
    niter <- niter + 1
    if (niter > maxiter){
      stop ("\r It is very hard to place random samples without overlap. 
  ...
  if(showplots){
    ...
    if(shape == "rectangle") {
      ...
      plotCircle(x = xs, y = ys, r = size[1], bg = rgb(0,1,0,0.4))
    }
    #par(oldpar)
  }
